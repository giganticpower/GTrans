...
def plt_fig(test_img, scores, img_scores, f_vis, gts, threshold, cls_threshold, save_dir, class_name):
    ...
    for i in range(num):
        ...
        fig_img.subplots_adjust(wspace=0.05, hspace=0.01)
        for ax_i in ax_img:
            for ax_ij in ax_i:
        ...
        ax_img[1, 0].imshow(gt, cmap='gray')  # 标签
        # ax_img[1, 0].title.set_text('GroundTruth')
        ax_img[2, 0].imshow(heat_map, cmap='jet', norm=norm, interpolation='none')  # 分割热力图
        # ax_img[2, 0].imshow(img, cmap='gray', alpha=0.7, interpolation='none')
        # ax_img[2, 0].imshow(vis_img, cmap='jet', alpha=0.7, interpolation='none') # 画线
        # ax_img[2, 0].title.set_text('Segmentation')
        ...
        ax_img[0, 1].imshow(f1[i] * 255, cmap='jet', norm=norm1, interpolation='none')
        # ax_img[0, 1].imshow(img, cmap='gray', alpha=0.7, interpolation='none')
        # ax_img[0, 1].title.set_text('Map0_56×56')
        ...
        ax_img[2, 1].imshow(f3[i] * 255, cmap='jet', norm=norm3, interpolation='none')
        # ax_img[2, 1].imshow(img, cmap='gray', alpha=0.7, interpolation='none')
        # ax_img[2, 1].title.set_text('Map2_14×14')
        #
        #
        ax_img[0, 2].imshow(f4[i] * 255, cmap='jet', norm=norm4, interpolation='none')
        # ax_img[0, 2].imshow(img, cmap='gray', alpha=0.7, interpolation='none')
        # ax_img[2, 0].title.set_text('Map 01')

        ax_img[1, 2].imshow(f5[i] * 255, cmap='jet', norm=norm5, interpolation='none')
